...
系统以 OOP 架构、向量化计算和增量数据更新为核心设计原则，每日收盘后自动选股并生成本地 HTML 报告。
...
```
配置加载 (.env) → 数据增量同步 (8进程) → 顺序执行7个策略 → 生成 HTML 报告
```
...
4. **生成报告** — 通过 `HtmlNotifier` 将全部策略结果（含空结果策略）渲染为单个自包含 HTML 文件（默认 `data/report.html`），含雪球行情链接与 baostock 解析的股票名称
